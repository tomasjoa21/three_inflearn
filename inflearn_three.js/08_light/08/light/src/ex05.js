...
// ----- 주제: 빛 라이트 광원 SpotLight 원뿔광원 무대조명, dat.GUI, AxesHelper, lightHelper
...
export default function example() {
	...
	// Light (SpotLight는 원뿔광원 무대조명 스포트라이트)
	// 인수 (색상(0xffffff or 'white'), 강도(1), 거리(0), 원뿔각도)
	const light = new THREE.SpotLight('white', 1, 30, Math.PI / 6);
	light.position.x = -5;
	light.position.y = 3; //light.position.set(2, 2, 0);
	scene.add(light);
	...
	// 라이트가 빛이 그림자를 만들어주는 속성
	light.castShadow = true;
	// 그림자의 해상도를 높여주는 속성 (성능에 영향을 준다)
	light.shadow.mapSize.width = 1024; // default: 512
	light.shadow.mapSize.height = 1024; // default: 512
	// 그림자의 테두리에 블러를 주는 속성
	light.shadow.radius = 5;
	// 그림자의 생기는 범위를 조절하는 속성
